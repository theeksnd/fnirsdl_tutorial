...
if __name__ == '__main__':   
    ...
    for train_idx, test_idx in kf.split(subjects): #iterate over the subjects in a LOSO evaluation. 
        ...
        # remove time-shift augmentations
        filter_train = []
        ...
        # validaiton data
        train_data, validation_data_ = train_test_split(np.array(filter_train), test_size=0.10, random_state=16)

        # from validation data,remove f-augs
        validation_data = []
        for file in validation_data_:
            if 'frq0.5' in file:
