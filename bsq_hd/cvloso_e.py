...
class MSTCN_WRAP(nn.Module):
    def __init__(self, num_stages=4, num_layers=8, num_f_maps=16, ())=200, num_classes=2):
...
if __name__ == '__main__':   
    ...
    for REMOVE_SUB in meta_events[0].keys(): #iterate over the subjects in a LOSO evaluation. 
        ...
        # train: freq-augmentation only
        filtered_train = []
        for file in train_data:
            if 'aug' not in file:
                filtered_train.append(file)
        train_data=np.array(filtered_train)
        ...
        # Parameters
        params = {'batch_size': 8, #we changed it for smooth-training 
                'shuffle': True,
                'num_workers': 4}
        # Generators
